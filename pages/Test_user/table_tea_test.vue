        ...
        <template>
            <v-dialog v-model="dialog_select_date" max-width="350px">
                <v-card class="px-3 text-center" style="background-color: rgba(247, 245, 245, 0.212)">
                    ...
                    <v-date-picker v-model="date1" multiple :events="arrayEvents" show-adjacent-months
                        event-color="green lighten-1"></v-date-picker> <!------:allowed-dates="allowedDates"------>
                    <v-card-actions>
                        <v-spacer></v-spacer>
                                ...
                                if (hourmatchData.exists() && !this.checkbox) {
                                    ...
                                    for (const keydate in this.date1) {
                                        for (const key in this.time_standart_sum) {
                                            db.ref(`Time_teacher/${this.value}/${this.date1[keydate]}/${this.time_standart_sum[key]}/`).orderByValue().equalTo(data.IDstu).once("value")
                                                .then(snapshot => {
                                                    if (snapshot.exists()) {
                                                        textadd = textadd.concat(' ', this.time_standart_sum[key])
                                                        isSave++;
                                                    }
                                                    else {
                                                        return db.ref(`Time_teacher/${this.value}/${this.date1[keydate]}/${this.time_standart_sum[key]}/`).once("value");
                                                    }

                                                    if (this.time_standart_sum.length == parseInt(key) + 1 && textadd.length != 0) {
                                                ...
                                                .then(snapshot => {
                                                    if (snapshot) {
                                                        ...
                                                        if (maxKey >= selectedObject.bath) {
                                                            text = text.concat(" ", this.time_standart_sum[key]);
                                                            isSave++;
                                                        }
                                                        if (this.time_standart_sum.length == parseInt(key) + 1 && text.length != 0) {
                                                            alert('เต็มแล้ว' + text);
                                                        }
                                                        if (maxKey < selectedObject.bath && this.time_standart_sum.length == parseInt(key) + 1) {
                                                            console.log('send save');
                                                        }
                                                        console.log('WorkData', maxKey, selectedObject.bath, this.time_standart_sum.length, parseInt(key) + 1, isSave);
                                    ...
                                    if (isSave == 0) {
                                        ...
                                        for (const keydate in this.date1) {
                                            for (const key in this.time_standart_sum) {
                                                const snapshot = db.ref(`Time_teacher/${this.value}/${this.date1[keydate]}/${this.time_standart_sum[key]}/`).once("value");
                                                if (selectedObject.key == '-NcQsHB9vgG53lJKPA-i') {
                                                    for (let x = 0; x < this.LimitedClass_all[0].bath; x++) {
                                                        db.ref(`Time_teacher/${this.value}/${this.date1[keydate]}/${this.time_standart_sum[key]}/`).update({
                                                            [x]: data.IDstu
                                                        });
                                                    }
