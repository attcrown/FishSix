    ...
    <div class="pt-5">
      <v-row align="center">
        ...
        <div>
          <v-text-field
            label="รหัสครู"
            placeholder="กรุณากรอกรหัสครูที่ต้องการค้นหา"
            v-model="teacherIdSearch"
          >
          </v-text-field>
          <v-btn class="text-white" color="black" @click="search_date_teacher">
            ค้นหาตารางครู
          </v-btn>
        </div>
        <v-col cols="12">
          ...
          >
                ...
                >
                  <template v-slot:activator="{}">
                    <v-btn
                      ...
                      dark
                      color="#322E2B"
                      class="mb-2 mt-5 hide-on-mobile"
                      @click="
                        ;(dialog_detail = true),
                          (mode = 'save'),
                          clear_item(),
            ...
            <template v-slot:item.actions="{ item }">
              ...
              >
                <v-icon class="text-h5" color="#26415B"> mdi-pencil </v-icon>
              </v-btn>
              <v-btn
                text
                icon
                ...
                v-if="!(parseInt(item.invite) > 0)"
              >
                <v-icon class="text-h5" color="#AD382F"> mdi-delete </v-icon>
              </v-btn>
            </template>
          </v-data-table>
        </v-col>
                ...
                <v-row>
                  ...
                  <v-col cols="12">
                    <v-autocomplete
                      ...
                      @change="
                        check_time_start(),
                          search_subject_tea(value),
                          search_style_tea(value),
                          save_detail.style == null
                      "
    ...
    <template>
      <v-dialog v-model="dialog_select_date" max-width="350px">
        <v-card
          ...
          style="background-color: rgba(247, 245, 245, 0.212)"
        >
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              dark
              class="mb-5"
              @click=";(dialog_select_date = false), (mode = 'save')"
            >
              ยืนยัน
            </v-btn>
            <v-spacer></v-spacer>
...
export default {
  data: () => ({
    ...
    rules: {
      name: [
        (val) => (val || '').length > 0 || 'กรุณาระบุข้อมูล',
  ...
  methods: {
        ...
        for (const key in childData) {
          let obj = {
            [key]: {
              ...
              location: childData[key].location,
            },
          }
          item = { ...item, ...obj }
        }
    ...
    async save_detail_data() {
      ...
      const selectedClass = this.LimitedClass_all.find((item) =>
      ...
      for (const date in this.date1) {
        ...
        if (selectedClass.key == '-NcQsFxCcoNS-uwmKUqE') {
          const Work_tea = await db
            .ref(`date_teacher/${this.value}/${this.date1[date]}/`)
            .once('value')
          ...
          if (Work_tea.exists()) {
            ...
            for (const key in Work_data_tea) {
              if (
                Work_data_tea[key].Class.key == '-NcQsFxCcoNS-uwmKUqE' &&
                Work_data_tea[key].style_subject == this.save_detail.style &&
                Work_data_tea[key].subject == this.save_detail.subject
              ) {
                //ตรวจสอบ FlipClass
                console.log('GOOD', Work_data_tea[key], key)
                time_data_tea = this.validateTime_save(
                  Work_data_tea[key].start,
                  Work_data_tea[key].stop
                )
                ...
                if (anyInA) {
                  ...
                  db.ref(
                    `date_teacher/${this.value}/${this.date1[date]}/${key}`
                  ).remove()
            ...
            db.ref(
              `date_teacher/${this.value}/${this.date1[date]}/${time_sum[0]}E${
                time_sum[time_sum.length - 1]
              }`
            ).update({
            ...
            db.ref(
              `date_teacher/${this.value}/${this.date1[date]}/${this.picker_start}E${this.picker_stop}`
            ).update({
              // idclass : this.generateRandomId(10),
              Class: selectedClass,
              createAt: new Date(),
        ...
        //------------PRIVATE------------
          ...
          if (Work_tea.exists()) {
            ...
            for (const key in Work_data_tea) {
              if (
                Work_data_tea[key].Class.key == '-NcQsHB9vgG53lJKPA-i' &&
                Work_data_tea[key].style_subject == this.save_detail.style &&
                Work_data_tea[key].subject == this.save_detail.subject
              ) {
                //ตรวจสอบ FlipClass
    ...
    search_style_tea(item) {
      ...
      db.ref(`user/${item}/classLocation`).on('value', (snapshot) => {
        ...
        for (const key in childData) {
          // console.log(">>>", childData[key]);
          const getLocationPromise = db
            .ref(`location/${childData[key]}`)
            .once('value')
          Promise.all([getLocationPromise]).then(([location]) => {
            const location_name = location.val()
            this.style_subject.push({
              key: childData[key],
    ...
    async search_date_teacher() {
      const db = this.$fireModule.database()
      this.isLoading = true
      let snapshot = []
      let childData = []
      let teacherDataSnapshot = []

      try {
        if (this.teacherIdSearch) {
          teacherDataSnapshot = await db
            .ref('user/')
            .orderByChild('teacherId')
            .equalTo(this.teacherIdSearch.toUpperCase())
            .once('value')
          const teacherData = teacherDataSnapshot.val()
          snapshot = await db
            .ref(`date_teacher/`)
            .child(Object.keys(teacherData)[0])
            .once('value')
          childData = snapshot.val()

          childData = { [snapshot.key]: childData }
        } else {
          snapshot = await db.ref(`date_teacher/`).orderByKey().once('value')
          childData = snapshot.val()
        }
        ...
        this.arrayEvents = []
        let item = []
        console.log(this.roughSizeOfObject(childData))
        for (const key in childData) {
            ...
            if (parseInt(new Date(date).getTime()) >= parseInt(now.getTime())) {
              ...
              for (const time in datedata) {
                ...
                if (
                  ...
                  const getSubjectPromise = db
                    .ref(`subject_all/${timedata.subject}`)
                    .once('value')
                  const getLocationPromise = db
                    .ref(`location/${timedata.style_subject}`)
                    .once('value')
                  Promise.all([
                    getTeacherPromise,
                    getSubjectPromise,
                    ...
                    if (true) {
                      //this.search_value == key && this.search_style_sub == timedata.style_subject && this.search_class == timedata.class) {
                      item.push({
    ...
    check_time_start() {
      const db = this.$fireModule.database()
      db.ref(`Time_teacher/${this.value}/${this.date1}`).on(
        'value',
        (snapshot) => {
          if (snapshot.exists()) {
            this.time_standart = []
            const childData = snapshot.val()
            const details = Object.keys(childData).map((key) =>
              key.substring(2, 7)
            )
    ...
    deleteItemConfirm() {
      ...
      const db = this.$fireModule.database()
      db.ref(
        `date_teacher/${this.delcon.key}/${this.delcon.date}/${this.delcon.time_s}E${this.delcon.time_e}`
      ).remove()
      this.delcon = []
      this.closeDelete()
      this.clear_item()
    ...
    search_teacher() {
      ...
      try {
        if (this.status.includes('teacher')) {
          console.log('เฉพาะครู')
          db.ref(`user/${this.keyuser}`).on('value', (snapshot) => {
            const childData = snapshot.val()
            this.items.push({
              key: this.keyuser,
              name: childData.teacherId + ' ครู' + childData.nickname,
            })
          })
        } else {
          db.ref('user/')
            ...
            .once('value', (snapshot) => {
              ...
              for (const key in childData) {
                item.push({
                  key: key,
                  name:
                    childData[key].teacherId + ' ครู' + childData[key].nickname,
                })
              }
              this.items = item
              console.log(this.roughSizeOfObject(childData))
