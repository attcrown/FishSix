<template>
  <div class="container-fluid">
    ...
    <v-row v-if="!isLoading">
      ...
      <div>
        <v-text-field label="รหัสนักเรียน" v-model="studentIdSearch">
        </v-text-field>
        <v-btn class="text-white" color="black" @click="searchStudent">
          ค้นหานักเรียน
        </v-btn>
      </div>
      <div class="col-sm-12">
        ...
        <v-row class="text-center px-4 align-center" wrap>
            <v-col cols="12" md="12">
                <v-btn color="white" @click="prev" :disabled="(startAt==0)"> ก่อนหน้า </v-btn>
              ...
              <v-container>
                <v-row no-gutters>
                  ...
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    label="ชื่อจริงนักเรียน"
                    :disabled="isExportAll"
                    value="ชื่อจริงนักเรียน"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    ...
                    :disabled="isExportAll"
                    value="นามสกุลนักเรียน"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    label="ชื่อเล่นนักเรียน"
                    :disabled="isExportAll"
                    value="ชื่อเล่น"
                  ></v-checkbox>
                  <v-checkbox
                  ...
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    label="ชั่วโมงเรียนที่เรียนไปแล้ว"
                    :disabled="isExportAll"
                    value="ชั่วโมงเรียนที่เรียนไปแล้ว Flip class"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    label="ชั่วโมงเรียนที่เหลืออยู่"
                    :disabled="isExportAll"
                    value="ชั่วโมงเรียนที่เหลืออยู่ Flip class"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    label="ชั่วโมงเรียนที่แถม"
                    :disabled="isExportAll"
                    value="ชั่วโมงเรียนที่แถม Flip class"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    ...
                    :disabled="isExportAll"
                    value="วันที่ซื้อคอร์ส Flip class"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    ...
                    :disabled="isExportAll"
                    value="วันที่คอร์สหมดอายุ Flip class"
                  ></v-checkbox>
                  <p>&#x2022; Private class</p>
                  <v-checkbox
                    class="m-0"
                    ...
                    label="ชั่วโมงเรียนที่เรียนไปแล้ว"
                    :disabled="isExportAll"
                    value="ชั่วโมงเรียนที่เรียนไปแล้ว Private class"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    label="ชั่วโมงเรียนที่เหลืออยู่"
                    :disabled="isExportAll"
                    value="ชั่วโมงเรียนที่เหลืออยู่ Private class"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    label="ชั่วโมงเรียนที่แถม"
                    :disabled="isExportAll"
                    value="ชั่วโมงเรียนที่แถม Private class"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    label="วันที่ซื้อคอร์ส"
                    :disabled="isExportAll"
                    value="วันที่ซื้อคอร์ส Private class"
                  ></v-checkbox>
                  <v-checkbox
                    class="m-0"
                    v-model="selectedHeaders"
                    label="วันที่คอร์สหมดอายุ"
                    :disabled="isExportAll"
                    value="วันที่คอร์สหมดอายุ Private class"
                  ></v-checkbox>
                  <p>&#x2022; ข้อมูลผู้ปกครอง</p>
                  <v-checkbox
                    class="m-0"
...
export default {
    ...
    return {
      ...
      headers: [
        ...
        { text: 'นามสกุล', value: 'stu.lastName', sortable: false },
        {
          ...
          value: 'stu.parentMobile',
          sortable: false,
          dataType: 'string',
        },
        {
          text: 'จำนวนชั่วโมง Flip Class ที่เหลือ ',
          value: 'stu.hourLeft',
          sortable: true,
        },
        {
          text: 'จำนวนชั่วโมง Private Class ที่เหลือ ',
          value: 'stu.privateHourLeft',
          sortable: true,
        },
    ...
    isExportAll(newValue) {
      if (!newValue) {
        ...
        this.selectedHeaders = [
          ...
          'ระดับการศึกษา',
          'ประเภทคลาส',
          'ชั่วโมงเรียนที่เรียนไปแล้ว Flip class',
          'ชั่วโมงเรียนที่เหลืออยู่ Flip class',
          'ชั่วโมงเรียนที่แถม Flip class',
          'วันที่ซื้อคอร์ส Flip class',
          'วันที่คอร์สหมดอายุ Flip class',
          'ชั่วโมงเรียนที่เรียนไปแล้ว Private class',
          'ชั่วโมงเรียนที่เหลืออยู่ Private class',
          'ชั่วโมงเรียนที่แถม Private class',
          'วันที่ซื้อคอร์ส Private class',
          'วันที่คอร์สหมดอายุ Private class',
          'ชื่อผู้ปกครอง',
          'อาชีพผู้ปกครอง',
          'ความคาดหวัง',
          'เวลาที่บันทึกข้อมูล',
  ...
  methods: {
    ...
    generateRowData(item, selectedHeaders) {
      return selectedHeaders.map((header) => {
        switch (header) {
          ...
          case 'ประเภทคลาส':
            return Array.isArray(item.stu.classTypes)
              ? item.stu.classTypes.join(', ')
              : item.stu.classTypes
      ...
      return new Promise((resolve, reject) => {
        ...
        try {
          const selectedData = this.selectedItems.map((item) =>
            this.generateRowData(item, this.selectedHeaders)
          )
    ...
    async searchStudent() {
      ...
      if (this.studentIdSearch) {
        query = await db
          .ref('user/')
          .orderByChild('studentId')
          .equalTo((this.studentIdSearch).toUpperCase())
      ...
      query
        ...
        .then((snapshot) => {
          ...
          snapshot.forEach((childSnapshot) => {
            const key = childSnapshot.key
            const childData = childSnapshot.val()
            const dateFotmatted = childData.createdAt
              ? new Date(childData.createdAt).toLocaleString()
              : '-'
            const stu = {
              studentId: childData.studentId,
              createdAt: dateFotmatted,
              hourLeft: childData.hourLeft || '-',
              privateHourLeft: childData.privateHourLeft || '-',
              firstName: childData.firstName,
              lastName: childData.lastName,
