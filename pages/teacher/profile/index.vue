        ...
        <v-col cols="12">
          ...
          <v-col lg="10" md="10" sm="12" xs="12">
                ...
                <v-row class="px-4">
                    ...
                    <v-file-input
                      v-if="!idCardCopy || isEditingDetail"
                      label="อัพโหลดสำเนาบัตรประชาชน"
                      v-model="idCardCopyUpload"
                      accept=".pdf,image/*"
                    ></v-file-input>
                    <a
                      v-if="idCardCopy && !isEditingDetail"
                    ...
                    <table class="table table-sm">
                      <tbody>
                        <tr v-for="subject in subjects" :key="subject.name">
                          <td>{{ subject.name }}</td>
                          <td
                            v-for="(level, index) in subject.level"
                            :key="index"
                          >
                            <v-checkbox
                              class="m-0"
                              :input-value="
                                isChecked(subject.name, level, subject.key)
                              "
                              @click="
...
export default {
  ...
  data() {
    return {
      ...
      contracts: ['พนักงานประจำ', 'ครูสาขา', 'ครูออนไลน์'],

      workTypes: ['Full Time', 'Part Time'],
      classTypes: ['Flip class', 'Private'],
      classLocations: [],
      ...
      rateRules: [(value) => !!value || 'กรุณากรอกเรทค่าจ้าง'],
      startDateRules: [(value) => !!value || 'กรุณาเลือกวันที่เริ่มทำงาน'],
      teacherIdRules: [
        (value) => !!value || 'กรุณากรอก รหัสครู',
        (value) =>
          /^FS\d{4}$/.test(value) ||
          'รูปแบบ รหัสครู ไม่ถูกต้อง (ต้องเป็น FS ตามด้วยตัวเลข 4 หลัก)',
      ],
    }
  },
  computed: {
 ...
 this.fullName();
  ...
  methods: {
    ...
    async getTeacherLocation() {
      ...
      for (let i = 0; i < this.classLocation.length; i++) {
        const classSnapshot = await db
          .ref(`location/${this.classLocation[i]}`)
          .once('value')
    ...
    isChecked(subjectName, level, key) {
      const selectedSubject = this.selectedSubjects.find(
